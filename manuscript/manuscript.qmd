...
One notable feature of IMF loan outcomes is that they are *left-censored* at zero—that is, loan amounts, participation rates, and the number of conditions are often zero, but cannot take negative values. In such cases, ordinary least squares (OLS) estimation yields biased and inconsistent results. The Tobit model addresses this issue by explicitly modeling the latent continuous variable underlying the censored observed outcomes [@tobin1958], and is particularly well-suited to our setting [@amemiya1984].
...
$$
\mathbf{P}_{it} = \left( \ln(\text{GDP}_{it}), \ln(\text{GDP}_{it})^2, \ln(\text{PCGDP}_{it}), \ln(\text{PCGDP}_{it})^2 \right)^\top
$$
